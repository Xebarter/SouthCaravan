...
function AdminCategoriesSettings() {
  ...
  async function resetToDefaults() {
    const ok = confirm('Reset all categories to defaults? This will remove all custom categories and subcategories.');
    if (!ok) return;
  ...
  function RowControls({ item }: { item: CategoryRow }) {
    return (
      <div className="flex items-center gap-2 shrink-0">
        ...
        <Switch checked={item.is_active} onCheckedChange={(checked) => toggleActive(item, checked)} aria-label={`Toggle ${item.name}`} />
        <Button
          variant="ghost"
          size="icon"
      ...
      .map((parent) => {
        ...
        const matchedSub = allSub
          .map((sub) => {
            ...
            const subMatch = sub.name.toLowerCase().includes(query) || sub.slug.toLowerCase().includes(query);
            if (subMatch) return { ...sub, _leaves: leaves };
            const matchedLeaves = leaves.filter(
              (leaf) => leaf.name.toLowerCase().includes(query) || leaf.slug.toLowerCase().includes(query),
            );
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Cartegories & menu taxonomy</h2>
          <p className="text-muted-foreground mt-1">
            Control marketplace visibility and storefront navigation: top-level menu items (level 1), subcategories, and the
            same taxonomy used when creating products.
          </p>
        </div>
      ...
      <Card id="seed-taxonomy" className="border-border/50">
        <CardHeader className="pb-3">
          <CardTitle>Default Cartegories</CardTitle>
          <CardDescription>Load the default categories and subcategories shown in the storefront menu.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="destructive" onClick={resetToDefaults} disabled={resetting || seeding}>
              {resetting ? (
      ...
      <Card id="add-category" className="border-border/50">
        <CardHeader className="pb-3">
          <CardTitle>Add new category / top-level menu item</CardTitle>
          <CardDescription>
            Level 1 entries are the parent categories shown in the sidebar. For level 2, use quick add under each level 1 row
            or choose level and parent here.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-4 lg:grid-cols-5">
          <Input placeholder="Display name" value={formName} onChange={(event) => setFormName(event.target.value)} />
      ...
      {loading ? (
        ...
        <Card className="border-border/50">
          <CardHeader>
            ...
            <CardDescription>Toggle visibility, sort order, or remove entries.</CardDescription>
          </CardHeader>
                  ...
                  <div key={parent.id} className="rounded-lg border border-border overflow-hidden">
                    <div className="w-full px-3 py-2.5 bg-card/40 flex items-center justify-between gap-3 hover:bg-secondary/40 transition-colors">
                      <button
                        ...
                        onClick={() => setExpandedParentIds((prev) => ({ ...prev, [parent.id]: !expanded }))}
                      ...
                      <div className="p-3 space-y-3">
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            className="h-8"
                          />
                          <Button size="sm" onClick={() => quickAddLevel2(parent)} disabled={quickAddingParentId === parent.id}>
    ...
    <main className="flex-1 overflow-auto">
      <div className="container mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs
          ...
          onValueChange={(v) => {
            setActiveTab(v);
            router.replace(`/admin/settings?tab=${encodeURIComponent(v)}`, { scroll: false });
          }}
          className="space-y-4"
        >
          <TabsList>
