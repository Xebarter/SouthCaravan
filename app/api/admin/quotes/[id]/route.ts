...
async function assertPlatformQuoteAccess(quoteId: string): Promise<
...
> {
  ...
  if (!platform.ok) {
    return {
      ok: false,
      response: NextResponse.json({ error: platform.error }, { status: 503 }),
    };
  }

  const { data: quote, error } = await supabaseAdmin.from('quotes').select('id,vendor_user_id').eq('id', quoteId).maybeSingle();
  if (error) {
    console.error('[admin/quotes/[id] assert]', error);
    return { ok: false, response: NextResponse.json({ error: error.message }, { status: 500 }) };
  }
  if (!quote) return { ok: false, response: NextResponse.json({ error: 'Not found' }, { status: 404 }) };

  if (String(quote.vendor_user_id) !== platform.userId) {
    return { ok: false, response: NextResponse.json({ error: 'Not found' }, { status: 404 }) };
  }

  return { ok: true, platformUserId: platform.userId };
}
...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const gate = await assertPlatformQuoteAccess(id);
  if (!gate.ok) return gate.response;

  const { data: quote, error } = await supabaseAdmin.from('quotes').select('*').eq('id', id).maybeSingle();
  if (error) {
    console.error('[admin/quotes/[id] GET]', error);
  ...
  if (iErr) {
    console.error('[admin/quotes/[id] GET items]', iErr);
  ...
  if (productIds.length > 0) {
    const { data: rows, error: pErr } = await supabaseAdmin
      ...
      .in('id', productIds);
    if (pErr) {
      console.error('[admin/quotes/[id] GET products]', pErr);
    } else {
      products = rows ?? [];
    }
    const invalid = (products ?? []).some((p) => {
      const v = p.vendor_id == null ? '' : String(p.vendor_id).trim();
      return v !== '' && isUuid(v);
    });
    if (invalid) {
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: quote, error: lErr } = await supabaseAdmin.from('quotes').select('*').eq('id', id).maybeSingle();
  if (lErr) {
    console.error('[admin/quotes/[id] PATCH lookup]', lErr);
  ...
  if (eiErr) {
    console.error('[admin/quotes/[id] PATCH items load]', eiErr);
  ...
  if (productIds.length > 0) {
    const { data: owned, error: oErr } = await supabaseAdmin
      ...
      .in('id', productIds);
    if (oErr) {
      console.error('[admin/quotes/[id] PATCH products]', oErr);
      return NextResponse.json({ error: oErr.message }, { status: 500 });
    }
    for (const p of owned ?? []) {
      const vid = p.vendor_id ? String(p.vendor_id).trim() : '';
      if (vid !== '' && isUuid(vid)) {
        return NextResponse.json({ error: 'This request is not a platform listing quote.' }, { status: 403 });
      }
    }
  }
  ...
  if (lineItems.length > 0) {
    ...
    for (const row of existingItems ?? []) {
      ...
      const { error: uErr } = await supabaseAdmin
        ...
        .eq('quote_id', id);
      if (uErr) {
        console.error('[admin/quotes/[id] PATCH item]', uErr);
  ...
  if (rErr) {
    console.error('[admin/quotes/[id] PATCH refresh]', rErr);
  ...
  const { data: updated, error: qErr } = await supabaseAdmin
    ...
    .update(patch)
    .eq('id', id)
    .eq('vendor_user_id', gate.platformUserId)
    .select('*')
    .single();

  if (qErr) {
    console.error('[admin/quotes/[id] PATCH quote]', qErr);
