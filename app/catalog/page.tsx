...
export default function CatalogPage() {
  ...
  const subcategories = useMemo(() => {
    ...
    return [
      'all',
      ...new Set(mockProducts.filter((p) => p.category === selectedCategory).map((p) => p.subcategory).filter(Boolean)),
    ];
  }, [selectedCategory]);
                  ...
                  <CardContent className="pb-3 flex-1 flex flex-col">
                    ...
                    <div className="p-3 bg-secondary/50 rounded-lg mb-4">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <span key={i} className={i < Math.floor(vendor?.rating || 0) ? 'text-primary text-xs' : 'text-muted text-xs'}>
                              ★
                            </span>
                          ))}
                        </div>
