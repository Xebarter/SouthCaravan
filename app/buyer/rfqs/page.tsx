...
export default function BuyerRfqsPage() {
  ...
  const [rfqs, setRfqs] = useState<ApiQuoteRow[]>([]);
  ...
  const filtered = useMemo(() => rfqs.filter((q) => q.buyer_id === buyerId), [rfqs, buyerId]);
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-foreground">My RFQs</h1>
            <p className="text-sm text-muted-foreground mt-2">
              Create RFQs, track vendor responses, and convert accepted quotes into orders.
            </p>
          </div>
        ...
        <Card className="rounded-2xl border-border/70 bg-card/60 backdrop-blur shadow-sm">
          <CardHeader className="pb-3">
            ...
            <CardDescription>Your submitted RFQs and their latest status</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {loading ? (
              ...
              <div className="rounded-2xl border-2 border-dashed border-border/60 bg-muted/20 p-10 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-2">Create your first RFQ to request a price from a vendor.</p>
              </div>
            ) : (
              filtered.map((q) => {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="min-w-0">
                        <p className="font-medium truncate">
                          RFQ <span className="font-mono text-muted-foreground">#{formatShortId(q.id)}</span>
                        </p>
